...
export default function FavoritesPage() {
  ...
  const params = useMemo(() => {
    const p: Record<string, string> = { favorited: 'true' }
  ...
  const fetchMovies = () => {
    ...
    api.get('/api/movies/', { params })
      ...
      .catch(err => { if (active) setError(err?.message ?? 'Failed to load favorites') })
  ...
  const toggleFavorite = async (movieId: number, isFav?: boolean) => {
    try {
      ...
      setMovies(prev => prev.filter(m => m.id !== movieId || !isFav).map(m => m.id === movieId ? { ...m, is_favorite: !isFav } : m))
      // If unfavorited, we can also refetch to drop it from the list
      if (isFav) fetchMovies()
    } catch (err: any) {
      if (err?.response?.status === 401) navigate('/login')
    }
  }

  if (loading) return <section className="container"><div className="card">Loading favorites…</div></section>
  ...
  return (
    <section className="container slide-up" style={{paddingTop: 24, paddingBottom: 24}}>
      <h2 style={{margin:'8px 0 16px', letterSpacing: 0.2}}>
        <span style={{marginRight:8}}>⭐</span> My Favorites
      </h2>
        ...
        <div className="adv-search">
          ...
          {showAdvanced && (
            <div className="adv-grid">
              {/* Sort, Order, Status, Tags, Duration, Rating, MPA blocks copied from Movies */}
