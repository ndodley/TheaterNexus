...
export default function MyReviewsPage() {
  ...
  async function deleteReview(movieId: number) {
    if (!confirm('Delete your review for this movie?')) return
    try {
      await api.delete(`/api/reviews/movies/${movieId}/me/`)
      setReviews(prev => prev.filter(r => r.movie !== movieId))
    } catch (err: any) {
      ...
      {reviews.map(rv => (
        <div key={rv.id} className="card" style={{padding:12, marginBottom:12}}>
          ...
          <div style={{display:'flex', gap:8}}>
            ...
            <button onClick={() => deleteReview(rv.movie)} style={{ backgroundColor: '#eee', color: '#333' }}>Delete</button>
          </div>
        </div>
      ))}
    </section>
