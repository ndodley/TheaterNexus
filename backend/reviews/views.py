...
class MovieMyReviewsListView(generics.ListAPIView):
	serializer_class = ReviewSerializer
	permission_classes = [permissions.IsAuthenticated]

	def get_queryset(self):
		movie_id = self.kwargs.get('movie_id')
		return Review.objects.filter(movie_id=movie_id, user=self.request.user).select_related('movie').order_by('-updated_at')
